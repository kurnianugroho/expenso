...
struct DashboardChartView: View {
    ...
    var trends: [MonthlyExpenseModel]
    var startDate: Date
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                Chart {
                    ForEach(0 ..< trends.count, id: \.self) { i in
                        LineMark(
                            x: .value("Index", trends[i].index),
                            y: .value("Total", trends[i].total)
                        )
                ...
                .chartXScale(
                    domain: -0.3 ... Double(trends.count - 1) + 0.3,
                    range: .plotDimension(padding: 0)
                )
                .chartXAxis {
                    AxisMarks(values: trends.map { $0.index }) { value in
                        AxisGridLine()
                        AxisTick()
                        AxisValueLabel {
                            if let x = value.as(Int.self),
                               let idx = trends.firstIndex(where: { $0.index == x })
                            {
                                Text(trends[idx].date.formattedMonth())
                            }
                        }
                    }
                }
